...
class YOLOSegDetector:
    """
    YOLOv8 segmentation via ONNX Runtime (QNN NPU or CPU).
    Output format matches YOLODetector (bbox + class + confidence + center) for drop-in use.
    Seg model has extra mask outputs; we parse detections the same way; mask decoding can be added later.
    """
    ...
    def __init__(self, model_path: str, qnn_dll_path: str | None = None, confidence_threshold: float = 0.45):
        ...
        self._has_mask_proto = len(outs) >= 2

    def preprocess(self, frame: np.ndarray) -> np.ndarray:
        """Resize to 640x640, BGR->RGB, normalize, NCHW."""
        img = cv2.resize(frame, (self._input_size, self._input_size))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = img.astype(np.float32) / 255.0
        img = img.transpose(2, 0, 1)
    ...
    def detect(self, frame: np.ndarray) -> list[dict]:
        """
        Run inference. Returns same shape as YOLODetector.detect():
        [{"class", "confidence", "bbox", "center"}, ...]
        """
        import time
        h_orig, w_orig = frame.shape[:2]
        inp = self.preprocess(frame)
        ...
        # Seg: output0 (1, 116, 8400) -> 4 bbox + 80 class + 32 mask coeffs
        raw = out[0]
        if raw.ndim == 3:
            raw = raw[0]
        raw = np.transpose(raw, (1, 0))  # (8400, 116)
        scale_x = w_orig / self._input_size
        scale_y = h_orig / self._input_size
...
# Single-class segmentation (e.g. crack). Output shape (1, 4+num_classes+32, 8400); for 1 class = (1, 37, 8400).
...
class YOLOCrackSegDetector:
    ...
    def detect(self, frame: np.ndarray) -> list[dict]:
        ...
        if raw.ndim == 3:
            raw = raw[0]
        # (37, 8400) or (4+num_classes+32, 8400)
        raw = np.transpose(raw, (1, 0))
        num_cols = raw.shape[1]
        # 4 bbox + num_classes (1 for crack) + 32 mask coeffs
